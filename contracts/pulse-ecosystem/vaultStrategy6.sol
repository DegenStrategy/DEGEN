...
interface IEmit {
    ...
    function checkPeriodFinish() external view returns (bool);
...
contract tokenVault is ReentrancyGuard {
	...
	address public emitChef = 0x6F0BD602147437F2Ce031dE0809b25a721552148;
	address public rewardToken = 0xefD766cCb38EaF1dfd701853BFCe31359239F305;
    ...
    constructor() {
        stakeToken = IERC20(0x32fB5663619A657839A80133994E45c5e5cDf427);
        poolID = 9;
	token = ;
    ...
    function deposit(uint256 _amount, address referral) external nonReentrant {
        ...
        harvest();
		//solves for tax token
		uint256 _beforeAmount = stakeToken.balanceOf(address(this));
		stakeToken.safeTransferFrom(msg.sender, address(this), _amount);
		_amount = stakeToken.balanceOf(address(this)) - _beforeAmount;
		...
		// Solves deposit fee
		uint256 _before = IEmit(emitChef).balanceOf(address(this));
		IEmit(emitChef).stake(stakeToken.balanceOf(address(this)));
		uint256 _after = IEmit(emitChef).balanceOf(address(this));
    ...
    function harvest() public {
		...
		uint256 _amount = IEmit(emitChef).balanceOf(address(this));
		accDtxPerShare+= _accumulatedRewards * 1e12  / (_amount - vaultBalance);
    }

	function initialize() external {
		uint256 _amount = IEmit(emitChef).rewards(address(this));
    ...
    function withdraw(uint256 _stakeID) public nonReentrant {
		...
		IEmit(emitChef).withdraw(user.amount);
	...
	function collectVaultsCommission() public {
		IEmit(emitChef).withdraw(vaultBalance);
		vaultBalance = 0;
