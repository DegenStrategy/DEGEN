...
contract NFTmarketplace is ERC721Holder, ReentrancyGuard {
    ...
    event SetNftForSale(uint256 saleId, address token, uint256 tokenId, uint256 plsPrice, uint256 dtxPrice, address indexed seller);
    event NftSold(uint256 offeringId, address indexed buyer, address forToken, uint256 provision);
    event UpdateSale(bool cancled, uint256 saleId, uint256 pricePls, uint256 priceDtx);
    ...
    function setNftForSale(address _token, uint256 _tokenId, uint256 _plsPrice, uint256 _dtxPrice) external {
        require (_plsPrice !=0 || _dtxPrice !=0, "price must be non-null");
        IERC721(_token).safeTransferFrom(msg.sender, address(this), _tokenId);
        nftOfferings.push(
                NFTsale(_token, _tokenId, _plsPrice, _dtxPrice, msg.sender, true)
            );

        emit SetNftForSale(nftOfferings.length-1, _token, _tokenId, _plsPrice, _dtxPrice, msg.sender);
    }
    ...
    function swapNftPls(uint256 _saleId) external payable nonReentrant {
        ...
        require(sale.plsPrice != 0, "only swap for DTX is allowed");
    ...
    function swapNft(uint256 _saleId) external nonReentrant {
        ...
        fee = amount * provision / 10000;
        amount-=fee;
        require(sale.dtxPrice != 0, "only swap for PLS is allowed");
        require(IERC20(dtxToken).transferFrom(msg.sender, treasury, fee));
        require(IERC20(dtxToken).transferFrom(msg.sender, sale.owner, amount));
        
        sale.isValid = false;

        IERC721(sale.token).safeTransferFrom(address(this), msg.sender, sale.tokenId);

        emit NftSold(_saleId, msg.sender, dtxToken, fee);
    }
    ...
    function updateSale(uint256 _saleId, uint256 _plsPrice, uint256 _dtxPrice) external nonReentrant {
        ...
        emit UpdateSale(false, _saleId, _plsPrice, _dtxPrice);
    }
