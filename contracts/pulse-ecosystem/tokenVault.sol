...
contract tokenVault is ReentrancyGuard {
	...
	IERC20 public immutable stakeToken; // Staking token
     ...
     */
    function deposit(uint256 _amount, address _poolInto, uint256 _threshold, uint256 _fee) external nonReentrant {
    ...
    */
	...
	function multiCall(address _user, uint256 _stakeID) external view returns(uint256, uint256, uint256) {
		...
		return(user.amount, stakeToken.balanceOf(address(this)), _pending);
	}
